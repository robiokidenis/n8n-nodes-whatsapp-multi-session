				...
				if (resource === 'session') {
					if (operation === 'list') {
						...
						try {
							...
							const qrCode = response.body?.data?.qr_code || response.body?.qr_code || response.body?.qrCode;
							...
							returnData.push({
								json: {
									...
									qrCodeDataUrl: qrCode || '', // Data URL format (after image format) - use this in <img src="...">
					...
					if (operation === 'sendText') {
						...
						const result = await handleMessageSending(
							`${baseUrl}/api/sessions/${sessionId}/send`,
							{
								to: phoneNumber,
								message: messageText,
							},
						...
						const caption = this.getNodeParameter('caption', i, '') as string;
						const result = await handleMessageSending(
							`${baseUrl}/api/sessions/${sessionId}/send-image`,
							{
								to: phoneNumber,
								image_url: imageUrl,
								caption: caption,
						...
						const result = await handleMessageSending(
							`${baseUrl}/api/sessions/${sessionId}/send-document`,
							{
								to: phoneNumber,
						...
						const result = await handleMessageSending(
							`${baseUrl}/api/sessions/${sessionId}/send-location`,
							{
								to: phoneNumber,
								latitude: latitude,
								longitude: longitude,
						...
						const result = await handleMessageSending(
							`${baseUrl}/api/sessions/${sessionId}/forward`,
							{
								to: phoneNumber,
						...
						const result = await handleMessageSending(
							`${baseUrl}/api/sessions/${sessionId}/reply`,
							{
								to: phoneNumber,
								message: replyText,
